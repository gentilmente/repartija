...
class Hash
  def to_html
    list_items = map do |k, v|
      html_value = v.respond_to?(:to_html) ? v.to_html : "<span>$ #{v}</span>"
      ["<li>#{k}: ", html_value, '</li>']
    end

    ['<ul>', list_items, '</ul>'].join
  end
end
...
configure do
  ...
  # para acceder en WLAN a la IP del server http://192.168.0.10:4567
  set :bind, '0.0.0.0'
end
...
helpers do
  ...
  def hash_to_html(hash, opts = {})
    ...
    hash.each do |key, value|
      ...
      if value.is_a?(Hash)
        out += "\n" + hash_to_html(value, indent_level: indent_level + 2)
        + ' ' * (indent_level + 2) + "</li>\n"
      else
